...
### Selective bundle plans are component-only

New `hotwire:bundle-preset` outputs use plan schema v2. The plan records only `preset`, `components`, the effective
`modules` and a normalized artifact `hash`; controllers no longer select package visual modules or enter generated plans.
After upgrading, replay a bundle's recorded component selection without reconstructing its original command:

```bash
php artisan hotwire:bundle-preset --from=resources/css/hotwire.css --force
```

Existing v1 plans remain readable and can be migrated by that command, but `hotwire:check --fix` does not rewrite them
automatically because they have no hash proving the application left the generated bytes untouched. V2 package drift is
auto-fixable while its hash remains valid. External edits are preserved and shown as a diff; move intended custom CSS to
a separate application stylesheet before regenerating. Invalid plans and removed presets require manual recreation.
