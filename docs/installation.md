...
| `--preset=<name>`          | Select the shipped `nova` or `bloom` CSS preset discovered from the package's public entrypoints. Defaults to `nova`; local presets created by `hotwire:make-preset` are imported manually.                                    |
