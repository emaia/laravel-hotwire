...
1. **Copies stubs** to `resources/` — `app.css`, `app.js`, the Stimulus bootstrap (`libs/stimulus.js`) and the
   controllers loader (`controllers/index.js`). The CSS scaffold activates the selected official preset and leaves every
   other official preset beside it as a commented import; Nova is selected by default.
