...
| `id`          | `string\|object\|null`      | derived       | Stable identifier used in the toolbar's outlet selector. Accepts a model, defaults to `\Emaia\LaravelHotwire\Support\FieldKey::toId($name)` (so `user[bio]` becomes `user-bio`), and falls back to a deterministic render-scoped id. Inherited from scoped Field context when absent. |
