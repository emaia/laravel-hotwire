...
| `dialog--modal#clickOutside` | Closes when clicking outside (use with `click` event on overlay) |
| `dialog--modal#showLoading` | Shows the loading template before a Turbo request |
...
<div
    ...
    data-dialog--modal-visible-class="opacity-100"
    data-dialog--modal-backdrop-hidden-class="opacity-0"
    data-dialog--modal-backdrop-visible-class="opacity-50"
    data-dialog--modal-dialog-hidden-class="opacity-0 scale-95"
    data-dialog--modal-dialog-visible-class="opacity-100 scale-100"
...
>
    ...
    <div class="fixed inset-0 flex items-center justify-center">
        ...
        >
            ...
            <button type="button" data-action="dialog--modal#close">Close</button>
        </div>
    </div>
</div>

<!-- Trigger -->
<button type="button" data-action="dialog--modal#open">Open modal</button>
```
...
```html
<div data-controller="dialog--modal" ...>
    <div class="fixed inset-0 bg-black/50" data-dialog--modal-target="backdrop"></div>

    <div class="fixed inset-0 flex items-center justify-center">
        <div data-dialog--modal-target="dialog" class="bg-white rounded-lg shadow-xl">
            <turbo-frame
                id="modal-content"
                data-dialog--modal-target="dynamicContent"
            ></turbo-frame>
        </div>
    </div>
</div>
...
```javascript
document.querySelector('[data-controller="dialog--modal"]').addEventListener("modal:opened", (event) => {
