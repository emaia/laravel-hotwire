...
low-level controller used by `<hw:toast>` and can also be rendered directly when needed.
...
- `@emaia/sonner`
- The `toaster` controller initialized on the page (creates the Sonner container).
...
For session flash messages, use [`<hw:toast>`](../../components/toast.md), which reads the
