...
2. **Before the render** (`turbo:before-render`/relevant `turbo:before-frame-render`): while armed, captures the `FileList`
...
3. **After the render** (`turbo:render`/relevant `turbo:frame-render`): if the re-rendered form has a field marked
...
descendant it contains. Unrelated sibling frames cannot consume an armed preservation cycle. A controller mounted on a
page-level wrapper still observes descendant frame renders.
