...
Generate a selective CSS bundle for an explicit set of components and controllers.
...
At least one selection is required. Components automatically include their mounted controllers and all selections pull
in transitive visual dependencies. Substrate controllers accept slash notation, such as `turbo/progress`.
...
file is reported but not overwritten. CSS selection or application preset problems require manual changes or explicit
regeneration.

In an interactive terminal, the command can offer safe fixes. `--fix` publishes or updates package-owned controller
files, regenerates supported loader metadata and adds missing npm dependencies without prompting. When dependencies are
added, the package manager runs unless `--skip-install` is present. Run the application's production asset build
separately; this command validates configuration and contracts, not compiled assets.
