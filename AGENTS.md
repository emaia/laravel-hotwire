...
## New/updated commands (since `0.32.0`)

| Command               | Description |
|-----------------------|-------------|
| `hotwire:sync-loader` | Idempotent command that regenerates `resources/js/controllers/index.js` with the current exclusion list of com-dep controllers. Used in CI/production builds as a build step. In local development the sync runs automatically via `LoaderSync` in the ServiceProvider boot. |
...
- **Tier zero-dep** — ~40 controllers with no npm dependencies are auto-loaded from the vendor directory
  via `import.meta.glob` in the generated `controllers/index.js`. No `hotwire:controllers` step needed.
- **Tier com-dep** — ~10 controllers with npm dependencies (carousel, chart, map, rich-text, file-upload,
  input-mask, timeago, toast/toaster, tooltip) are **excluded** from the vendor glob and still require
  opt-in via `hotwire:controllers <name>`.
- The exclusion list is generated by `Support\LoaderSync` based on the `npm` key in `src/Registry/catalog.php`.
  Adding a new com-dep controller only requires listing the npm packages in the catalog — the exclusion
  pattern is derived automatically from the controller filename.
