...
it('renders a template with the replace action by default', function () {
    $view = $this->blade('<x-hw::optimistic target="post_1_favorite">OK</x-hw::optimistic>');
...
it('wraps the slot inside a <template> tag', function () {
    $view = $this->blade('<x-hw::optimistic target="x"><button>Done</button></x-hw::optimistic>');
...
it('supports custom turbo stream action', function () {
    $view = $this->blade('<x-hw::optimistic target="list" action="append">X</x-hw::optimistic>');

    $view->assertSee('data-optimistic-action="append"', false);
});

it('supports a CSS selector via targets prop', function () {
    $view = $this->blade('<x-hw::optimistic targets=".todo" action="remove">X</x-hw::optimistic>');
...
it('omits target attributes when both are empty (for refresh action)', function () {
    $view = $this->blade('<x-hw::optimistic action="refresh">X</x-hw::optimistic>');
...
it('escapes target id to prevent attribute injection', function () {
    $view = $this->blade('<x-hw::optimistic :target="$id">X</x-hw::optimistic>', [
...
it('preserves data-field markers in the slot for client-side population', function () {
    $view = $this->blade('
        <x-hw::optimistic target="messages" action="append">
            <article>
