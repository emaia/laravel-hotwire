...
it('renders with default props', function () {
    $view = $this->blade('<x-hwc-confirm title="Delete item?">Content</x-hwc-confirm>');
...
it('renders the trigger slot', function () {
    $view = $this->blade('
        <x-hwc-confirm title="Are you sure?">
            <x-slot:trigger>
...
it('renders the message when provided', function () {
    $view = $this->blade('<x-hwc-confirm title="Delete?" message="This cannot be undone.">Content</x-hwc-confirm>');

    $view->assertSee('This cannot be undone.');
});

it('does not render message element when empty', function () {
    $view = $this->blade('<x-hwc-confirm title="Delete?">Content</x-hwc-confirm>');

    $view->assertDontSee('mt-2 text-sm text-gray-600', false);
});

it('renders custom confirm and cancel labels', function () {
    $view = $this->blade('<x-hwc-confirm title="Send?" confirm-label="Send" cancel-label="Go back">Content</x-hwc-confirm>');
...
it('applies custom confirm class', function () {
    $view = $this->blade('<x-hwc-confirm title="Delete?" confirm-class="bg-red-600 hover:bg-red-700 text-white">Content</x-hwc-confirm>');
...
it('sets custom id', function () {
    $view = $this->blade('<x-hwc-confirm id="my-confirm" title="Delete?">Content</x-hwc-confirm>');
...
it('registers with custom prefix', function () {
    ...
    expect($resolved)->toHaveKey('custom-confirm');
});
