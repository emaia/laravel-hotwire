...
it('uses explicit for attribute', function () {
    $view = $this->blade('<x-hw::field.label for="email">E-mail</x-hw::field.label>');
...
it('derives for from name prop', function () {
    $view = $this->blade('<x-hw::field.label name="email">E-mail</x-hw::field.label>');

    $view->assertSee('for="email"', false);
});

it('derives for from name with bracket notation', function () {
    $view = $this->blade('<x-hw::field.label name="variables[0][name]">Variables</x-hw::field.label>');

    $view->assertSee('for="variables-0-name"', false);
});

it('does not render for attribute without name or for', function () {
    $view = $this->blade('<x-hw::field.label>Generic</x-hw::field.label>');
...
it('omits for when the slot wraps an input (implicit labeling)', function () {
    $view = $this->blade('<x-hw::field.label name="switch"><input type="checkbox" />Airplane</x-hw::field.label>');

    $view->assertDontSee('for=', false);
});

it('omits for when the slot wraps an x-hw::input', function () {
    $view = $this->blade('<x-hw::field.label name="switch"><x-hw::input type="checkbox" name="switch" />Airplane</x-hw::field.label>');

    $view->assertDontSee('for=', false);
});

it('omits for when the slot wraps a select', function () {
    $view = $this->blade('<x-hw::field.label name="plan"><select><option>A</option></select> Plan</x-hw::field.label>');

    $view->assertDontSee('for=', false);
});

it('omits for when the slot wraps a textarea', function () {
    $view = $this->blade('<x-hw::field.label name="bio"><textarea></textarea> Bio</x-hw::field.label>');
...
it('respects explicit for even when slot wraps a control', function () {
    $view = $this->blade('<x-hw::field.label for="custom"><input type="text" />Inner</x-hw::field.label>');
...
it('uses value prop as content', function () {
    $view = $this->blade('<x-hw::field.label for="email" value="My Label" />');

    $view->assertSee('My Label');
});

it('prefers slot over value when both provided', function () {
    $view = $this->blade('<x-hw::field.label for="email" value="From prop">From slot</x-hw::field.label>');
...
it('renders required marker when required', function () {
    $view = $this->blade('<x-hw::field.label for="email" required>E-mail</x-hw::field.label>');
...
it('uses custom required label', function () {
    $view = $this->blade('<x-hw::field.label for="email" required required-label="(obrigatório)">E-mail</x-hw::field.label>');
...
it('merges custom class on the label element', function () {
    $view = $this->blade('<x-hw::field.label for="email" class="text-sm font-bold">E-mail</x-hw::field.label>');
...
it('passes through arbitrary attributes', function () {
    $view = $this->blade('<x-hw::field.label for="email" data-test="x">E-mail</x-hw::field.label>');
