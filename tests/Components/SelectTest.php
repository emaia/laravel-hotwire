...
it('renders a select element', function () {
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\', 2 => \'Inactive\']" />');
...
it('renders empty select when no options', function () {
    $view = $this->blade('<x-hw::select name="status" />');
...
it('derives id from bracket notation', function () {
    $view = $this->blade('<x-hw::select name="variables[0][status]" :options="[1 => \'Active\']" />');

    $view->assertSee('id="variables-0-status"', false);
});

it('uses explicit id', function () {
    $view = $this->blade('<x-hw::select name="status" id="my-select" :options="[1 => \'Active\']" />');

    $view->assertSee('id="my-select"', false);
});

it('generates random id when name is absent', function () {
    $view = $this->blade('<x-hw::select :options="[1 => \'Active\']" />');

    $view->assertSee('id="hw-select-', false);
...
it('marks the selected option', function () {
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\', 2 => \'Inactive\']" :selected="2" />');
...
it('merges selected with old() input', function () {
    ...
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\', 2 => \'Inactive\']" :selected="1" />');
...
it('disables old() when :old=false', function () {
    ...
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\', 2 => \'Inactive\']" :selected="1" :old="false" />');
...
it('uses old() with derived dot-notation key for array names', function () {
    ...
    $view = $this->blade('<x-hw::select name="variables[0][status]" :options="[\'active\' => \'Active\', \'inactive\' => \'Inactive\']" />');
...
it('renders placeholder option when provided', function () {
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\']" placeholder="Choose..." />');
...
it('keeps placeholder selected when options have integer key 0 and nothing is chosen', function () {
    $view = $this->blade('<x-hw::select name="status" placeholder="-- Selecione --" :options="[\'Aberto\', \'Fechado\']" />');
...
it('does not select placeholder when a value is selected', function () {
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\']" placeholder="Choose..." :selected="1" />');
...
it('renders blank option when nullable and no placeholder', function () {
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\']" :nullable="true" />');
...
it('does not select blank option when nullable and value is selected', function () {
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\']" :nullable="true" :selected="1" />');

    $view->assertSee('value="1" selected', false);
});

it('uses placeholder text when both nullable and placeholder are provided', function () {
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\']" :nullable="true" placeholder="Choose..." />');
...
it('uses explicit error key override', function () {
    ...
    $view = $this->blade('<x-hw::select name="status" error-key="custom.path" :options="[1 => \'Active\']" />');

    $view->assertSee('aria-invalid="true"', false);
});

it('sets aria-required when required attribute is present', function () {
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\']" required />');
...
it('merges custom class on select element', function () {
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\']" class="w-full" />');
...
it('passes through arbitrary attributes', function () {
    $view = $this->blade('<x-hw::select name="status" :options="[1 => \'Active\']" disabled data-test="x" />');
...
it('renders multiple attribute when set', function () {
    $view = $this->blade('<x-hw::select name="ids[]" :options="[1 => \'A\', 2 => \'B\']" multiple />');

    $view->assertSee('multiple', false);
});

it('marks multiple options as selected when selected is an array', function () {
    $view = $this->blade('<x-hw::select name="ids[]" :options="[1 => \'A\', 2 => \'B\', 3 => \'C\']" :selected="[1, 3]" multiple />');
...
it('marks no options as selected when selected array is empty with multiple', function () {
    $view = $this->blade('<x-hw::select name="ids[]" :options="[1 => \'A\', 2 => \'B\']" :selected="[]" multiple />');

    $view->assertDontSee('selected', false);
});

it('omits placeholder option when multiple is set', function () {
    $view = $this->blade('<x-hw::select name="ids[]" :options="[1 => \'A\']" placeholder="Choose..." multiple />');
...
it('omits nullable blank option when multiple is set', function () {
    $view = $this->blade('<x-hw::select name="ids[]" :options="[1 => \'A\']" :nullable="true" multiple />');

    $view->assertDontSee('value=""', false);
});

it('coerces scalar selected into array when multiple', function () {
    $view = $this->blade('<x-hw::select name="ids[]" :options="[1 => \'A\', 2 => \'B\']" :selected="2" multiple />');
...
it('merges multiple selected with old() input array', function () {
    ...
    $view = $this->blade('<x-hw::select name="ids[]" :options="[1 => \'A\', 2 => \'B\', 3 => \'C\']" :selected="[1]" multiple />');
...
it('picks up name and required from field via @aware', function () {
    $view = $this->blade('
        <x-hw::field name="status" required>
            <x-hw::select :options="[1 => \'Active\', 2 => \'Inactive\']" />
        </x-hw::field>
    ');
