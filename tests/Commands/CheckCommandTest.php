...
it('accepts visual coverage from any generated CSS bundle', function () {
    writeView('page.blade.php', '<x-hw::badge>New</x-hw::badge>');
    $this->artisan('hotwire:bundle-preset --components=modal --output=resources/css/front.css --no-interaction')->assertSuccessful();
    $this->artisan('hotwire:bundle-preset --components=badge --output=resources/css/admin.css --no-interaction')->assertSuccessful();
...
it('does not reinterpret an explicitly selected generated bundle as a complete preset', function () {
    $this->artisan('hotwire:bundle-preset --components=badge --output=resources/css/presets/layout.css --no-interaction')
        ->assertSuccessful();
...
it('keeps a generated bundle under the presets directory in selective mode', function () {
    writeView('page.blade.php', '<x-hw::badge>New</x-hw::badge>');
    $this->artisan('hotwire:bundle-preset --components=modal --output=resources/css/presets/layout.css --no-interaction')
        ->assertSuccessful();
...
it('does not require preset coverage for the standalone Toaster controller', function () {
    ...
    expect($exit)->toBe(0)
        ->and(Artisan::output())->not->toContain('toaster  not covered by any generated CSS bundle');
});
...
it('refuses to fix an incompatible lazy loader without changing application files', function () {
    writeView('page.blade.php', '<div data-controller="modal"></div>');
    writePackageJson(['devDependencies' => [
        '@emaia/stimulus-lazy-loader' => '^1.1.0',
    ]]);
    ...
    $package = File::get($this->packageJsonPath);
    $stub = File::get($this->targetDir.'/index.js');
    ...
    expect(File::get($this->packageJsonPath))->toBe($package)
        ->and(File::get($this->targetDir.'/index.js'))->toBe($stub);
});

it('refuses to infer policy from an unsupported generated loader', function (string $metadata) {
    writePackageJson(['devDependencies' => [
        '@emaia/stimulus-lazy-loader' => '^2.0.0',
    ]]);
    File::ensureDirectoryExists($this->targetDir);
    $stub = LoaderStub::MARKER."\n".$metadata."\nconst controllers = import.meta.glob('./*_controller.js');";
    File::put($this->targetDir.'/index.js', $stub);

    $exit = Artisan::call('hotwire:check --fix --skip-install --no-interaction');

    expect($exit)->toBe(1)
        ->and(Artisan::output())->toContain('unsupported or missing loader metadata')
        ->toContain('hotwire:install')
        ->toContain('--core-only or --with-deps');

    expect(File::get($this->targetDir.'/index.js'))->toBe($stub);
})->with([
    'no metadata' => '',
...
it('rejects an incompatible lazy loader even when no view uses Hotwire', function () {
    writePackageJson(['devDependencies' => [
        '@emaia/stimulus-lazy-loader' => '^1.1.0',
    ]]);
    File::ensureDirectoryExists($this->targetDir);
...
it('regenerates the loading policy while preserving its dependency selection', function (bool $includeAll) {
    ...
    $this->artisan('hotwire:check --fix --skip-install --no-interaction')->assertSuccessful();

    $content = File::get($this->targetDir.'/index.js');
    $policy = LoaderStub::policyFromContent($content);

    expect($content)->toContain('"version":3')
        ->and($policy->includeAllComDepControllers)->toBe($includeAll)
        ->and($policy->eagerControllerPaths)->toHaveKey('modal');

    if ($includeAll) {
        expect($policy->includedComDepControllers)->toContain('chart', 'map');
    } else {
        expect($policy->includedComDepControllers)->toBe(['chart']);
    }
})->with([true, false]);
...
it('reports a missing lazy loader dependency without adding it with --fix', function () {
    writePackageJson(['devDependencies' => []]);
    File::ensureDirectoryExists($this->targetDir);
    File::put(
        $this->targetDir.'/index.js',
    ...
    expect($exit)->toBe(1)
        ->and(Artisan::output())->toContain('@emaia/stimulus-lazy-loader missing')
        ->toContain('requires ^2.0.0');
