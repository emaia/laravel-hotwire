...
describe("public CSS presets", () => {
    ...
    test("compiles the application-owned Nova clone generated by make-preset", () => {
        const css = contract.outputs.novaClone;
        const source = contract.sources.novaClone;
        ...
        expect(source).not.toContain('@import "./nova/');
        expect(css).toContain("--background:");
        expect(css).toMatch(slotSelector("button"));
        expect(css).toMatch(carouselMechanic);
    });
    ...
    test("compiles shared forced-colors and print control fallbacks into every bundle", () => {
        for (const css of [
            ...
            contract.outputs.selective,
            contract.outputs.novaClone,
